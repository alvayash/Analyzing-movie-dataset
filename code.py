...
def explore_data(dataset, start, end, rows_and_columns=False):
    ...
    """
    j=dataset[start:end-1]
    [print(a) for a in j]

    if rows_and_columns==True:
        print('Total rows in the list : ',len(dataset))
        print('Total columns in the list : ',len(dataset[0]))
...
def duplicate_and_unique_movies(dataset, index_):
    ...
    print('Total Duplicate movie entries: ',len(duplicate_movies))
    print('Total Unique movie entries: ',len(unique_movies))
...
movies=list(x for x in movies if x!=movies[0])
